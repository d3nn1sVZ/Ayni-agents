...
const INIT_AGENTS: Agent[] = [
  { id: 'a1', name: 'TaxBot PE',   mission: 'Consultas tributarias para clientes peruanos',   status: 'active', satsSpent: 3400, queriesMade: 34, currentTask: 'Consultando régimen NRUS para cliente #447', lastActive: 'hace 12s' },
  { id: 'a2', name: 'DataSage',    mission: 'Asistencia de data science para el equipo',       status: 'paying', satsSpent: 2250, queriesMade: 30, currentTask: '⚡ Pagando 75 sats — XGBoost vs LightGBM',    lastActive: 'ahora' },
  { id: 'a3', name: 'LexAgent MX', mission: 'Soporte legal corporativo para startups MX',      status: 'idle',   satsSpent: 1800, queriesMade: 12, currentTask: 'Esperando siguiente consulta…',               lastActive: 'hace 3min' },
]

const INIT_ACTIVITY: Entry[] = [
  { ts: Date.now()-12000, agentName: 'TaxBot PE',   action: '¿Cuándo pago el IGV mensual a SUNAT?',   sats: 100, type: 'query' },
  { ts: Date.now()-14000, agentName: 'TaxBot PE',   action: '⚡ Pago liquidado — 100 sats en 156ms',   sats: 100, type: 'payment' },
  { ts: Date.now()-30000, agentName: 'DataSage',    action: 'Manejo de valores nulos con pandas',       sats: 75,  type: 'query' },
  { ts: Date.now()-32000, agentName: 'DataSage',    action: '⚡ Pago liquidado — 75 sats en 183ms',    sats: 75,  type: 'payment' },
  { ts: Date.now()-55000, agentName: 'LexAgent MX', action: '¿Cómo constituir una SAS en México?',     sats: 150, type: 'query' },
  { ts: Date.now()-58000, agentName: 'LexAgent MX', action: '⚡ Pago liquidado — 150 sats en 201ms',   sats: 150, type: 'payment' },
]

const QUERIES = [
  { id: 'a1', name: 'TaxBot PE',   q: 'Libros contables PYME',       sats: 100 },
  { id: 'a2', name: 'DataSage',    q: 'Regularización L1 vs L2',      sats: 75  },
  { id: 'a3', name: 'LexAgent MX', q: 'Diferencia SAPI vs SA de CV',  sats: 150 },
]
...
export default function HumanPage() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      setMood('paying')
      setAgents(prev => prev.map(a => a.id === q.id ? { ...a, status: 'paying', currentTask: `⚡ Pagando ${q.sats} sats — ${q.q}` } : a))
      setActivity(prev => [{ ts: Date.now(), agentName: q.name, action: q.q, sats: q.sats, type: 'query' as const }, ...prev].slice(0, 20))
      setTimeout(() => {
        setMood('happy')
        setAgents(prev => prev.map(a => a.id === q.id ? { ...a, status: 'active', satsSpent: a.satsSpent + q.sats, queriesMade: a.queriesMade + 1, currentTask: `Respuesta recibida`, lastActive: 'ahora' } : a))
        setTotal(p => p + q.sats)
    ...
    <div className="min-h-dvh flex flex-col">
      ...
      <section className="px-6 pt-24 pb-16 text-center border-b border-white/[0.06]">
        <div className="max-w-2xl mx-auto">
          <div className="label mb-6">Vista: Humano · Agent Economy</div>
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-tight mb-5">
            <span className="text-gradient">Tus agentes trabajan.</span><br />
            <span className="text-[#38BDF8]">Tus agentes pagan.</span>
          </h1>
          <p className="text-white/40 text-lg leading-relaxed mb-8">
            Desplegás el agente una vez. Él descubre servicios, paga en sats y te trae resultados.
            Sin contratos. Sin API keys. Sin fricción.
          </p>
          <div className="flex justify-center gap-4">
            {[
              { v: agents.filter(a => a.status !== 'idle').length, l: 'agentes activos',  c: 'text-emerald-400' },
              { v: totalSats.toLocaleString(),                      l: 'sats gastados',    c: 'text-[#E8B547]'   },
              { v: `$${(totalSats * 0.0000006).toFixed(4)}`,        l: 'costo total USD',  c: 'text-[#38BDF8]'   },
            ].map(s => (
      ...
      <section className="px-6 py-20 border-b border-white/[0.06]">
        <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <div className="label mb-5">Mis agentes desplegados</div>
              ...
              {agents.map(a => (
                <div key={a.id} className="glass p-5 flex flex-col gap-3">
                  ...
                  <div className="flex gap-5 text-xs">
                    ...
                    <div><span className="text-[#38BDF8] font-mono font-bold">{a.queriesMade}</span><span className="text-white/20 ml-1">consultas</span></div>
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              <div className="label">Actividad en tiempo real</div>
      ...
      <section className="px-6 py-20 text-center">
        <div className="max-w-xl mx-auto">
          <h3 className="text-2xl font-bold text-gradient mb-3">¿Contribuís conocimiento?</h3>
          <p className="text-white/30 text-sm mb-8">Mirá cómo ganás cada vez que un agente consulta tu tribu.</p>
          <div className="flex gap-3 justify-center">
            <Link href="/contributors" className="btn-gold">Ver contribuidores →</Link>
